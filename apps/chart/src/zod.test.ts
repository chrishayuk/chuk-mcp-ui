...
import { chartSchema } from "./zod";

describe("chart zod schema validation", () => {
  ...
  it("accepts pie chart with label+value objects", () => {
    const data = {
      type: "chart",
      version: "1.0",
      chartType: "pie",
  ...
  it("rejects missing chartType", () => {
    ...
    expect(chartSchema.safeParse(data).success).toBe(false);
  });
